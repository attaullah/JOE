...
def main(argv):
    ...
    model = train_utils.get_custom_model(FLAGS.network, data_config, FLAGS.weights, FLAGS.lt, FLAGS.opt, FLAGS.lr)
    os.environ['TF_FORCE_GPU_ALLOW_GROWTH'] = "true"
    ...
    train_utils.start_training(model, dso, FLAGS.epochs, FLAGS.semi, FLAGS.batch_size)
    ac = train_utils.log_accuracy(model, dso, FLAGS.lt, FLAGS.semi, labelling=FLAGS.lbl)
